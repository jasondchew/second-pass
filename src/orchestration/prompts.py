...
def check_understanding_prompt(
    ...
    system = (
        ...
        "the retrieval prompts they were given, and what they wrote by hand, "
        "identify two things: what they got right (be specific about which "
        "claim, not just 'good job'), and anything that's incorrect, "
        "incomplete, or worth double-checking. If the original material is "
        "thin or absent, rely on your own domain knowledge but say so if "
        "you're not fully certain about a flag. If everything looks solid, "
        "\"flags\" can be an empty list - don't invent a nitpick just to have one. "
        + SOURCE_RULES
...
def revise_entry_prompt(
    ...
    system = (
        ...
        "question - leave everything else exactly as the user wrote it. Use "
